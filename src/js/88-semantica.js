...
let _emb=null, semIndex=null;
function setEmbedder(fn){ _emb=fn; }   // testes/serviço hospedado: fn(texts:string[]) -> vetores normalizados
/* 🔒 SUPPLY-CHAIN (achado 8 do 2º report — fechado): o transformers.js vem de um CDN pinado,
   então ele NUNCA roda na página — roda num Web Worker (blob, type:module), que por
   construção não alcança localStorage (PAT/chaves) nem o DOM. O que o worker vê: os textos
   a embedar (conteúdo da brain, não-segredo), IndexedDB/Cache da origem (índice + cache do
   modelo, não-segredo). Verificado cross-browser via e2e/semantica-real.spec.mjs (REAL_EMB=1,
   modelo real). Mitigações que continuam: versão pinada + só o dono reindexar. */
const SEM_WORKER_SRC=[
  "let loading=null;",
  "function ensure(cdn, model){",
  "  if(!loading) loading=(async()=>{",
  "    const mod=await import(cdn);",
  "    if(mod.env){ mod.env.allowLocalModels=false; mod.env.useBrowserCache=true; }",
  "    return mod.pipeline('feature-extraction', model, {progress_callback:function(x){",
  "      self.postMessage({tick:true, status:x.status||'', file:x.file||'', pct:x.progress?Math.round(x.progress):null});",
  "    }});",
  "  })();",
  "  return loading;",
  "}",
  ...
  "  const d=e.data||{};",
  "  try{",
  "    const p=await ensure(d.cdn, d.model);",
  "    const out=[];",
  "    for(const t of d.texts){ const r=await p(t,{pooling:'mean',normalize:true}); out.push(Array.from(r.data)); self.postMessage({tick:true, status:'embed'}); }",
  "    self.postMessage({id:d.id, vecs:out});",
  "  }catch(err){ self.postMessage({id:d.id, error:String((err&&err.message)||err)}); }",
  "};",
].join("\n");
const SEM_STALL_MS=120000;   // sem NENHUM sinal do worker por 2min → erro claro (não trava eterno)
let _semW=null, _semWUrl=null, _semSeq=0, _semLastTick=0, _semWatch=null;
const _semPend=new Map();
function _semFail(err){
  for(const p of _semPend.values()) p.reject(err);
  _semPend.clear();
  if(_semWatch){ clearInterval(_semWatch); _semWatch=null; }
  if(_semW){ try{ _semW.terminate(); }catch(e){} _semW=null; }   // próxima chamada recria do zero
  if(_semWUrl){ URL.revokeObjectURL(_semWUrl); _semWUrl=null; }
}
function semWorker(){
  if(_semW) return _semW;
  _semWUrl=URL.createObjectURL(new Blob([SEM_WORKER_SRC], {type:"text/javascript"}));
  _semW=new Worker(_semWUrl, {type:"module"});
  _semLastTick=Date.now();
  _semW.onmessage=e=>{
    const d=e.data||{};
    _semLastTick=Date.now();
    if(d.tick){ window.__semProg=d; return; }   // progresso (download do modelo/embeds) — vivo
    const p=_semPend.get(d.id); if(!p) return;
    _semPend.delete(d.id);
    if(_semWUrl){ URL.revokeObjectURL(_semWUrl); _semWUrl=null; }   // já carregou; não precisa mais do blob
    if(d.error) p.reject(new Error(d.error));
    else { window.__semWorkerOk=true; p.resolve(d.vecs); }
  };
  _semW.onerror=ev=>{ _semFail(new Error("worker de embeddings falhou: "+((ev&&ev.message)||"erro ao carregar"))); };
  return _semW;
}
async function localEmbedder(texts){
  const id=++_semSeq;
  return new Promise((resolve,reject)=>{
    _semPend.set(id,{resolve,reject});
    semWorker().postMessage({id, texts, cdn:EMB_CDN, model:EMB_MODEL});
    if(!_semWatch) _semWatch=setInterval(()=>{
      if(!_semPend.size){ clearInterval(_semWatch); _semWatch=null; return; }
      if(Date.now()-_semLastTick>SEM_STALL_MS)
        _semFail(new Error("embeddings pararam de responder (rede lenta ou CDN/modelo indisponível) — tente de novo"));
    }, 15000);
  });
}
...
function askIndexStatus(){
  ...
  loadSemIndex().then(idx=>{
    if(!idx || !idx.items || !idx.items.length){ el.innerHTML=`<span style="color:var(--tx3)">sem índice ainda — clique <b>Reindexar</b> pra ligar a busca semântica (baixa o modelo multilíngue ~130MB na 1ª vez, depois é offline).</span>`; return; }
