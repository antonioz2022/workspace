...
function memSyncBannerHtml(res,pid){
  ...
  const box=inner=>`<div style="font-size:12px;color:var(--warn);background:rgba(245,165,36,.09);border:1px solid rgba(245,165,36,.28);border-radius:9px;padding:8px 11px;margin:2px 0 8px">${inner}
    <div style="margin-top:6px">${draftBtn}<button class="btn sm" onclick="copyProjectContext('${pid}')" title="Monta o prompt (commits + pendências + memória) pra a IA da sessão do repo reescrever a memória">📋 Gerar atualização pra IA</button></div></div>`;
